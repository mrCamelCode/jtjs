...
const JSDOC_ANNOTATION_COLOR = '#3d80e3';
...
async function main() {
  try {
    ...
    writeFile(`${outputFilePath}.md`, addStylingToJsDocAnnotations(md), {
      encoding: 'utf-8',
    });
  } catch (error) {
...
async function getMarkdown(reactDocGenOutputFilePath: string): Promise<string> {
  ...
  return [
    '**Disclaimer**: These docs are auto-generated from data produced by ' +
      ...
      "The docgen also isn't great when it comes to displaying unions and function signatures. " +
...
function addStylingToJsDocAnnotations(md: string): string {
  const docAnnotationPattern = /(@[^\s]*)/gim;

  return md.replace(
    docAnnotationPattern,
    `<span style="color: ${JSDOC_ANNOTATION_COLOR}; font-weight: bold">$1</span>`
  );
}
