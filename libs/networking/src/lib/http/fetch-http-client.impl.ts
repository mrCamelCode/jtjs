  ...
  implements IHttpClient<RequestInit, Request, Response, FetchRawResponseBody>
{
  ...
  protected getTreatedUri(uri: string): string {
  ...
  async makeRequest<ParsedBodyType = undefined>(
    ...
    try {
      const requestUri = this.getTreatedUri(uri);
  ...
  private getNormalizedHeaders(headers?: HeadersInit): HeadersInit {
