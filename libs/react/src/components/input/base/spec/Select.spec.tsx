...
let value = '1';
const onChangeSelection = jest.fn((option: string) => (value = option));
...
describe('Select', () => {
  ...
  describe('generateSelectOptions', () => {
    ...
    test('should not include an empty first option when allowEmpty is set to false', () => {
      ...
      expect(
        options.every((option) => (option as SelectOption<string>).value !== '')
      ).toBe(true);
    });
  ...
  test('can have option groups', () => {
    ...
    expect(
      (screen.getByTestId('select-option-group') as HTMLOptGroupElement).label
    ).toBe('Grouped Options');
