...
export type RadioOption<T> = Option<
  ...
  Omit<
    LabelledRadioProps,
    'onChangeChecked' | 'id' | 'checked' | 'value' | 'label' | 'children'
  >
>;
 ...
 */
export const LabelledRadioGroup = forwardRef<
  HTMLFieldSetElement,
...
>(
  (
    ...
    const labelMarkup = (
      <legend
        className={buildClassName(labelClassName, 'jtjs-radio-group-label')}
        {...otherLabelProps}
      >
        <InlineText
    ...
    return (
      <FormGroup
        role="radiogroup"
        className={buildClassName(className, 'jtjs-radio-group')}
        {...otherProps}
        ref={ref}
      >
        {[
